...
:initialize
docker run -v .\\:/tmp/workspace -v vscrm:/root/.vscode-server -e "DB_SERVER=localhost" -e "DB_PORT=64001" -e "DB_USER=user0" -e "DB_PASSWORD=HeLlo|12" -e "DB_DATABASE=apomtschedsys" -e "API_SERVER=localhost" -e "API_PORT=8088" -w /tmp/workspace --name dotnet --pull always -td mcr.microsoft.com/dotnet/sdk:8.0 && echo "dotnet" Created.
docker run -v .\\:/tmp/workspace -v mysqld:/var/lib/mysql -e "MYSQL_ROOT_PASSWORD=HeLlo|12" -e "MYSQL_DATABASE=apomtschedsys" -e "MYSQL_USER=user0" -e "MYSQL_PASSWORD=HeLlo|12" -p 64001:3306 -w /tmp/workspace/database --name mysql --pull always -td mysql:8.3.0-oraclelinux8 && echo "mysql" Created.
docker run -v .\\:/tmp/workspace -v vscrm:/root/.vscode-server -w /tmp/workspace/frontend/public/react-appointment-scheduler --name node --pull always -td node:22.9.0-bullseye && echo "node" Created.
exit /b 0